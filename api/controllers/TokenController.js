...
module.exports = {
  ...
  find: function (req, res) {
    //const userCriteria = {providerId: req.param('providerId'), appId: req.param('appId'), email: req.param('email')};
  ...
  findByAppEmail: function (req, res) {
    Application.findOne({ id: req.param('appId'), email: req.param('email') }, function (err, app) {
      if (err) return res.json(400, err);
      if (!app) return res.json(401, { success: false, message: 'Unauthorized: application does not exist' });
